...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (tokenProvider.validateToken(accessToken) && tokenProvider.validateExpiredToken(accessToken)) {
            SecurityContextHolder.getContext().setAuthentication(tokenProvider.getAuthentication(accessToken));
        }

        if (!tokenProvider.validateExpiredToken(accessToken) && tokenProvider.validateToken(accessToken)) {
            String refreshToken = getTokenFromHeader(request, REFRESH_HEADER);
            if (tokenProvider.validateToken(refreshToken) && tokenProvider.validateExpiredToken(refreshToken)) {
